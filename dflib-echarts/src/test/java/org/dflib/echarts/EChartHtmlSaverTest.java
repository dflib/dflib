...
public class EChartHtmlSaverTest {
    ...
    @Test
    public void embeddedTemplate() throws IOException {
        ElementIdGenerator idGenerator = ElementIdGenerator.sequential();
        ...
        StringWriter out = new StringWriter();
        ECharts.saver().defaultEmbeddedTemplate().loadECharts(true).title("Test Charts").save(out, p1, p2);

        String expected = new String(getClass().getResourceAsStream("charts-embedded1.html").readAllBytes(), StandardCharsets.UTF_8);
        assertEquals(expected, out.toString());
    }
    ...
    public void themeUrls() throws IOException {
        ...
        EChartHtml p1 = ECharts
                .chart(idGenerator)
                .theme("t1", "https://example.org/t1.js")
                .xAxis("x")
                .series(SeriesOpts.ofLine().name("Y1"), "y1").plot(df2);
        EChartHtml p2 = ECharts
                .chart(idGenerator)
                .theme("t2", "https://example.org/t2.js")
                .xAxis("x")
                .series(SeriesOpts.ofLine().name("Y2"), "y2").plot(df2);
