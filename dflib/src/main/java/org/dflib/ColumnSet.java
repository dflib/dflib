...
public interface ColumnSet {
    ...
    /**
     * Returns a DataFrame with merged columns produced from the expression that maps each row to an Iterable of values,
     * such as a List or a Set. Each element in each Iterable goes in its own column. For a ColumnSet with a defined
     * width, if the splitExp does not generate enough values to fill a given row, the tail of the row will be filled
     * with nulls, and if it generates values in excess of the row width, extra values are ignored. In a dynamic
     * ColumnSet, the total number of produced columns will be equal to the longest row generated by the splitExp.
     */
    DataFrame explode(Exp<? extends Iterable<?>> splitExp);

    /**
     * Returns a DataFrame with merged columns produced from the expression that maps each row to an array
     ...
     */
    DataFrame explodeArray(Exp<? extends Object[]> splitExp);
     ...
     */
    DataFrame selectExploded(Exp<? extends Iterable<?>> splitExp);
     ...
     */
    DataFrame selectExplodedArray(Exp<? extends Object[]> splitExp);
}
