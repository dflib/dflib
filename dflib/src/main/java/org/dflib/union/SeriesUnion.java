...
public class SeriesUnion {
    ...
    /**
     * Detects if all series are of the same primitive type.
     * Returns the common primitive type, or NONE if types are mixed or non-primitive.
     */
    private static PrimitiveType detectCommonPrimitiveType(Series<?>[] series) {
        if (series.length == 0) {
