 ...
 * @since 0.11
 */
public final class PerColumnComparators {
    ...
    public static IntComparator of(Series<?> column, boolean ascending) {
        return ascending
                ? (i1, i2) -> Comparators.nullsLastCompare((Comparable) column.get(i1), (Comparable) column.get(i2))
                : (i1, i2) -> Comparators.nullsLastCompare((Comparable) column.get(i2), (Comparable) column.get(i1));
    }
    ...
    public static <V extends Comparable<? super V>> IntComparator of(DataFrame df, RowToValueMapper<V> sortKeyExtractor) {
        ...
        Comparator<RowProxy> rowComparator = (c1, c2) -> Comparators.nullsLastCompare(sortKeyExtractor.map(c1), sortKeyExtractor.map(c2));
