...
public class SaveThenLoadTest {
    ...
    private DataFrame saveThenLoad(DataFrame save, String label, Consumer<HardwoodSaver> customizer) {

        Path path = outBase.resolve(label + ".parquet");
        HardwoodSaver saver = Hardwood.saver();
        ...
        DataFrame load = Hardwood.loader().load(path);
        assertNotNull(load);
        return load;
    }
