...
class TestParquetWriter {
    ...
    public static <T> Builder<T> of(Class<T> type, Path baseFolder) {
        return new Builder<>(type, baseFolder.resolve(UUID.randomUUID() + ".parquet"));
    }

    static final class Builder<T> extends ParquetWriter.Builder<T, Builder<T>> {
        ...
        private Builder(Class<T> type, Path out) {
            super(new LocalOutputFile(out));
            this.type = type;
            this.out = out;
        }

        public Builder<T> writer(BiConsumer<RecordConsumer, T> writer) {
            this.writer = writer;
            return this;
        }
